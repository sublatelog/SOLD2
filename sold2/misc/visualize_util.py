...
def plot_junctions(input_image, junctions, junc_size=3, color=None):
    ...
    for idx in range(num_junc):
        # Fetch one junction
        junc = junctions[idx, :]
        cv2.circle(image, tuple(np.flip(junc)), radius=junc_size, color=color, thickness=3)
...
def plot_line_segments(
    ...
    if not plot_survived_junc:
        ...
        for idx in range(num_junc):
            # Fetch one junction
            junc = junctions[idx, :]
            cv2.circle(image, tuple(np.flip(junc)), radius=junc_size, color=(0, 255., 0), thickness=3) 
            
    # Only plot the junctions which are part of a line segment
    else:
        for idx in range(segments.shape[0]):
            seg = np.round(segments[idx, :]).astype(np.int) # Already in HW format.
            cv2.circle(image, tuple(seg[:2]), radius=junc_size, color=(0, 255., 0), thickness=3)
            cv2.circle(image, tuple(seg[2:]), radius=junc_size,  color=(0, 255., 0), thickness=3)
...
def plot_line_segments_from_segments(
    ...
    for idx in range(line_segments.shape[0]):
        ...
        cv2.line(image, tuple(np.flip(seg[0, :])), tuple(np.flip(seg[1, :])), color=color, thickness=line_width)
