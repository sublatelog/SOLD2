...
def warp_points(points, homography):
    ...
    new_points = np.concatenate([points[..., [1, 0]], np.ones_like(points[..., :1])], axis=-1)
...
def keypoints_to_grid(keypoints, img_size):
    ...
    grid_points = keypoints.float() * 2. / torch.tensor(img_size, dtype=torch.float, device=device) - 1.
...
def get_dist_mask(kp0, kp1, valid_mask, dist_thresh):
    ...
    dist_mask = dist_mask.repeat(1, 1, b_size).reshape(b_size * n_points, b_size * n_points)
...
def get_common_line_mask(line_indices, valid_mask):
    ...
    common_mask = common_mask.repeat(1, 1, b_size).reshape(b_size * n_points, b_size * n_points)
