...
class WireframeDataset(Dataset):
    def __init__(self, mode="train", config=None):
        ...
        if not self.gt_source == "official":
            ...
            if not os.path.exists(self.gt_source):
                raise ValueError("[Error] The specified ground truth source does not exist.")
    ...
    def construct_dataset(self):
        ...
        if self._check_dataset_cache():
            print("\t Found filename cache %s at %s"%(self.cache_name, self.cache_path))
    ...
    def get_filename_dataset(self):
        ...
        for idx in range(len(image_paths)):
            ...
            if (not (os.path.exists(image_path) and os.path.exists(label_path))):
                raise ValueError("[Error] The image and label do not exist. %s"%(image_path))
                
            # Further verify mat paths for test split
            if self.mode == "test":
                mat_path = mat_paths[idx]
                if not os.path.exists(mat_path):
                    raise ValueError("[Error] The mat file does not exist. %s"%(mat_path))
    ...
    def convert_line_map(lcnn_line_map, num_junctions):
        """ Convert the line_pos or line_neg (represented by two junction indexes) to our line map. """
    ...
    def get_photo_transform(self):
        ...
        photometric_trans_lst = [
            getattr(photoaug, trans)(**conf) for (trans, conf) in zip(trans_lst, trans_config_lst)
        ]
    ...
    def get_line_points(self, 
        ...
        """ Sample evenly points along each line segments and keep track of line idx. """
        ...
        junc_indices = np.array([[i, j] for (i, j) in zip(*np.where(line_map)) if j > i])
        
        line_segments = np.stack([junctions[junc_indices[:, 0]],
                                  junctions[junc_indices[:, 1]]
                                 ], 
                                 axis=1)
        ...
        num_samples = np.minimum(line_lengths // self.config["min_dist_pts"], self.config["max_num_samples"])
        ...
        line_indices = np.concatenate([line_indices, np.zeros(self.config["max_pts"] - len(line_indices))], axis=0)
        line_points = np.concatenate([line_points, np.zeros((self.config["max_pts"] - len(line_points), 2), dtype=float)], axis=0)
    ...
    def train_preprocessing(self, data, numpy=False):
        ...
        if not(list(image_size) == self.config["preprocessing"]["resize"]):
            ...
            # In HW format
            junctions = (junctions * np.array(self.config['preprocessing']['resize'], np.float) / np.array(size_old, np.float))
    ...
    def train_preprocessing_exported(
        ...
        # Check homographic augmentation
        warp = (self.config["augmentation"]["homographic"]["enable"] and disable_homoaug == False)
    ...
    def preprocessing_exported_paired_desc(self, data, numpy=False, scale=1.):
        """ Train preprocessing for paired data for the exported labels for descriptor training. """
