...
class WunschLineMatcher(object):
    ...
    def forward(self, line_seg1, line_seg2, desc1, desc2):
        ...
        line_points1 = torch.tensor(line_points1.reshape(-1, 2), dtype=torch.float, device=device)        
        line_points2 = torch.tensor(line_points2.reshape(-1, 2), dtype=torch.float, device=device)
    ...
    def sample_salient_points(self, 
                              line_seg, 
                              desc, 
                              img_size,
        ...
        num_samples_lst = np.clip(
                                  line_lengths // self.min_dist_pts,
                                  2, 
                                  self.num_samples
                                 )
        ...
        for n in np.arange(2, self.num_samples + 1):
            ...
            cur_line_points = np.stack([line_points_x, line_points_y], axis=-1).reshape(-1, 2)
            
            # cur_line_points is of shape (n_cur_lines * sample_rate, 2)
            cur_line_points = torch.tensor(cur_line_points, dtype=torch.float, device=device)
            ...
            # np.take_along_axis():多次元配列のソート
            cur_line_points = np.take_along_axis(cur_line_points, best[..., None], axis=2)[:, :, 0]
    ...
    def sample_line_points(self, line_seg):
        ...
        # The number of samples depends on the length of the line
        num_samples_lst = np.clip(line_lengths // self.min_dist_pts, 2, self.num_samples)
    ...
    def needleman_wunsch(self, scores):
        ...
        The cost of the InDel operation is set to 0 by subtracting the gap penalty to the scores.
        
        Inputs:
            scores: a (B, N, M) np.array containing the pairwise scores of the elements to match.
        """
        b, n, m = scores.shape
